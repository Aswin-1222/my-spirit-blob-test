...
function TotalUniqueDataPointsTable() {
  const [totalUniqueDataPointsTable, settotalUniqueDataPointsTable] = useState([]);
  ...
  const getFilteredRows = () => {
    let selectedRows = totalUniqueDataPointsTable;
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/total_unique_data_points_table')
      .then(response => {
        console.log(response.data);
        settotalUniqueDataPointsTable(response.data.data);
      })
      .catch(error => {
        console.error(error);
      });
                  ...
                  <Table sx={{ minWidth: 650 }} size="small" aria-label="a dense table">
                    <TableHead>
                      <TableRow>
                        ...
                        <TableCell>
                          <TableSortLabel
                            active={sortConfig && sortConfig.columnKey === 'url_count'}
                            direction={sortConfig && sortConfig.columnKey === 'url_count' ? sortConfig.direction : 'asc'}
                            onClick={() => handleSort('url_count')}
                          >
                            <b>URL COUNT</b>
                          </TableSortLabel>
                        </TableCell>
                      ...
                      {getSortedRows()
                        ...
                        .map((row, index) => (
                          <TableRow key={index}>
                            ...
                            <TableCell>{row.url_count}</TableCell>
...
export default TotalUniqueDataPointsTable;
