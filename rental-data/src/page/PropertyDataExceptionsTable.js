...
function PropertyDataExceptionsTable() {
  const [propertyDataExceptionCountTable, setpropertyDataExceptionCountTable] = useState([]);
  ...
  const getFilteredRows = () => {
    let selectedRows = propertyDataExceptionCountTable;
  ...
  useEffect(() => {
    axios
      .get('http://localhost:5000/property_data_exceptions_table')
      .then((response) => {
        console.log(response.data);
        setpropertyDataExceptionCountTable(response.data.data);
      })
      .catch((error) => {
        console.error(error);
      });
                  ...
                  <Table sx={{ minWidth: 650 }} size="small" aria-label="a dense table">
                    <TableHead>
                      <TableRow>
                        ...
                        <TableCell>
                          <TableSortLabel
                            active={sortConfig && sortConfig.columnKey === 'building'}
                            direction={
                              sortConfig && sortConfig.columnKey === 'building' ? sortConfig.direction : 'asc'
                            }
                        ...
                        <TableCell>
                          <TableSortLabel
                            active={sortConfig && sortConfig.columnKey === 'datetime'}
                            direction={
                              sortConfig && sortConfig.columnKey === 'datetime' ? sortConfig.direction : 'asc'
                            }
                      ...
                      {getSortedRows()
                        ...
                        .map((row, index) => (
                          <TableRow key={index}>
                            ...
                            <TableCell>{renderStatus(row.building)}</TableCell>
                            <TableCell>{row.exceptions}</TableCell>
                            <TableCell>{formatTimestamp(row.datetime)}</TableCell>
...
export default PropertyDataExceptionsTable; 