...
function NewUrlOnboardedTable() {
  const [newUrlOnboardedTable, setnewUrlOnboardedTable] = useState([]);
  ...
  const getFilteredRows = () => {
    let selectedRows = newUrlOnboardedTable;
  ...
  useEffect(() => {
    axios
      .get('http://localhost:5000/new_url_onboarded_table')
      .then((response) => {
        console.log(response.data);
        setnewUrlOnboardedTable(response.data.data);
      })
      .catch((error) => {
        console.error(error);
      });
...
export default NewUrlOnboardedTable;
