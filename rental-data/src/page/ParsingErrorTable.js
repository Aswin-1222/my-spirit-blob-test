...
function ParsingErrorTable() {
  const [parsingErrorTable, setparsingErrorTable] = useState([]);
  ...
  const getFilteredRows = () => {
    let selectedRows = parsingErrorTable;
  ...
  useEffect(() => {
    axios
      .get('http://localhost:5000/parsing_error_table')
      .then((response) => {
        console.log(response.data);
        setparsingErrorTable(response.data.data);
      })
      .catch((error) => {
        console.error(error);
      });
...
export default ParsingErrorTable; 