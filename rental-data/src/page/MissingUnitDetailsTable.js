...
function MissingUnitDetailsTable() {
  const [missingUnitDetailsTable, setmissingUnitDetailsTable] = useState([]);
  ...
  const getFilteredRows = () => {
    let selectedRows = missingUnitDetailsTable;
  ...
  useEffect(() => {
    axios
      .get('http://localhost:5000/missing_unit_details_table')
      .then((response) => {
        console.log(response.data);
        setmissingUnitDetailsTable(response.data.data);
      })
      .catch((error) => {
        console.error(error);
      });
...
export default MissingUnitDetailsTable; 