...
function InvalidPropertyDataTable() {
  const [invalidPropertyDataTable, setinvalidPropertyDataTable] = useState([]);
  ...
  const getFilteredRows = () => {
    let selectedRows = invalidPropertyDataTable;
  ...
  useEffect(() => {
    axios
      .get('http://localhost:5000/invalid_property_data_table')
      .then((response) => {
        console.log(response.data);
        setinvalidPropertyDataTable(response.data.data);
      })
      .catch((error) => {
        console.error(error);
      });
...
export default InvalidPropertyDataTable; 