...
function NoDataFoundTable() {
  const [noDataFoundTable, setnoDataFoundTable] = useState([]);
  ...
  const getFilteredRows = () => {
    let selectedRows = noDataFoundTable;
  ...
  useEffect(() => {
    axios
      .get('http://localhost:5000/no_data_found_table')
      .then((response) => {
        console.log(response.data);
        setnoDataFoundTable(response.data.data);
      })
      .catch((error) => {
        console.error(error);
      });
